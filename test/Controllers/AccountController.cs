    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                // Lightweight auth cookie as a backup in case session isn't persisted
                // correctly in certain hosting environments (e.g., plain HTTP on AWS).
                Response.Cookies.Append("AuthUser", user.Username, new CookieOptions
                {
                    HttpOnly = true,
                    SameSite = SameSiteMode.Lax,
                    Secure = false, // you are running over HTTP on AWS right now
                    Path = "/"
                });
